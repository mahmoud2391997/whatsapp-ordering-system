...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const store = await fetchSallaStoreInfo(token.access_token);
    const merchantId = String(store?.data?.id ?? params.get('merchant') ?? params.get('merchant_id') ?? '');
    if (!merchantId) throw new Error('Salla did not return a merchant id');
